...
// Components
import { TrackCover, PlaylistCover, VotingCard } from './components/SharedComponents';
import MusicParserModal from './components/MusicParserModal';
import { CreateRoomModal, InviteModal, DeleteConfirmModal } from './components/Modals';
...
const VKMusicPlayer = () => {
    ...
    const proposeTrack = async (track) => {
        ...
        if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
            ...
            if (roomScenario === 'withoutVoting') {
                ...
                setRoomTracks(prev => [...prev, trackData]);
            }
        } else {
    ...
    const voteForTrack = (votingId, vote) => {
        ...
        if (voting.user_vote) {
            addSystemMessage(`Вы уже проголосовали ${voting.user_vote === 'yes' ? 'ЗА' : 'ПРОТИВ'} это предложение`);
            return;
        }
        ...
        if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
            ...
            wsRef.current.send(JSON.stringify({
                type: messageType,
                session_id: votingId,
                track_id: voting.track.id || voting.track.vk_id,
                vote: vote
            }));
            
            setActiveVotings(prev => prev.map(v => {
                if (v.id === votingId) {
                    return { ...v, user_vote: vote };
                }
                return v;
            }));
